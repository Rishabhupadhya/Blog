...
export function getCloudDevOpsPostBySlug(slug: string) {
  ...
  const filePath = path.join(process.cwd(), "src/content/cloud-devops", `${slug}.mdx`);
...
export function getRelatedCloudDevOpsPosts(currentSlug: string, limit = 3) {
  return getAllCloudDevOpsPosts()
