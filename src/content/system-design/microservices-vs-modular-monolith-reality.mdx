...
> Industry case studies (e.g., a Prime Video workload) reported **order‑of‑magnitude cost reductions** when consolidating a serverless microservices pipeline into a simpler, monolithic design—for that specific use case.
...
*Each arrow = an in‑process function call (except the initial HTTP and DB call).* 

---

## The Same Feature — Two Implementations

### Microservices (Distributed, production‑grade sketch)
...
> Key points: single deployable unit, ACID transaction, DB‑enforced idempotency, safe handling of external side‑effects, transactional outbox for event publication.
...
- The examples above are **sketches** to illustrate concerns and patterns, not drop‑in code.  
- Every architecture has trade‑offs. The goal is **fit‑for‑purpose**, not purity.
